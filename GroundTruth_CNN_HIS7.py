...
def main():
    ...
    def train(epoch, loss_tr, loss_te, train_full=False):
        ...
        if train_full == False:
            ...
            if epoch == Epoch - 1:
                with torch.no_grad():
                    MSE_running = 0; count = 0
                    for batch, (X_b, y_b) in enumerate(loader):
                        X_b, y_b = X_b.to(device), y_b.to(device)
                        y_pred = CNN(X_b).squeeze(-1)
                        mse_i = MSE(y_pred[:, 0], y_b.to(device)).item()
                        MSE_running += mse_i
                        count += 1
                        
                    print("Epoch:", epoch, "MSE for training is:", MSE_running/count)
        
        loss_tr.append(train_loss)
        return loss_tr, loss_te
    loss_tr, loss_te = [], []
    print("#=================================================================#")
    print("Start training for HIS7 dataset; model id is:", args.model_id)
